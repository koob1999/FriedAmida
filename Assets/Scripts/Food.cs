...
public class Food : MonoBehaviour
{
	...
	public FriedFood DoFry(Cooking.OilTemp oil)
	{
		...
		{
			Debug.Log("rush");
			friedFood = new FriedFood(Cooking.FriedFoodReview.good, this.calorie, IsLastFood);
		}
		//マイナスアイテムを取得していれば腐り状態
		else if (badItemCount >= 1)
		{
			Debug.Log("bad");
			friedFood = new FriedFood(Cooking.FriedFoodReview.bad, this.calorie, IsLastFood);
		...
		else
		{
			...
			{
				Debug.Log("row");
				friedFood = new FriedFood(Cooking.FriedFoodReview.raw, this.calorie, IsLastFood);
			}
			//それ以外なら揚げ失敗
			else
			{
				Debug.Log("usually");
				friedFood = new FriedFood(Cooking.FriedFoodReview.usually, this.calorie, IsLastFood);
			}
		}
		return friedFood;
	}
